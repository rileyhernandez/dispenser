...
class LoadCell:
    def __init__(self):
        ...
        for cell in range(len(self.cells)):
            self.cells[cell].setDeviceSerialNumber(716692)
    ...
    async def get_readings(self):
        coros = [self.get_cell_reading[cell] for cell in self.cells]
    ...
    def plot(self):
        all_readings = list(self.data.values())[:len(self.cells)]
        self.data['weights'] = [0*i for i in all_readings[0]]
        for cell in range(len(self.cells)):
            for reading in range(len(all_readings[cell])):
                self.data['weights'][reading] += all_readings[cell][reading]
        self.data['weights'] = list(map(lambda x:x-self.offset, self.data['weights']))
        
        plt.close()
        plt.plot(self.data['weights'])
